...
for genome in `ls *.cigar | sed 's/\.cigar//' | sort | uniq`; do
	...
	if [ ! -s ./pileup/$genome.pileup ]; then
	    ...
	    $scriptdir/ssaha_pileup/ssaha_pileup -solexa 1 -cons 1 ${genome}_cigar.dat /pseudospace1/davised/data/rf/fna_official_new/A44a.fna ${genome}_reads.fastq > ../pileup/$genome.pileup
